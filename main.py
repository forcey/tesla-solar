...
def vehicle_status(credentials):
    auth = {'Authorization': 'Bearer ' + credentials['access_token']}
    r = requests.get('https://owner-api.teslamotors.com/api/1/vehicles/1492931321740893/data_request/charge_state', headers=auth)
    return r.json()['response']
...
def main():
    ...
    while True:
        ...
        print("Current charging {}V * {}A = {}W".format(voltage, current_charging_amp, charging_power))
        ...
        if abs(charging_amp - current_charging_amp) > 1:
